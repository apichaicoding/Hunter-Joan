...
module.exports = async (guild, member) => {
  ...
  try {
    ...
    for (let i = 0; i < data.length; i += 4) {
      const gray = (data[i] + data[i + 1] + data[i + 2]) / 3; // ค่าสีเฉลี่ยจากสีแดง เขียว และน้ำเงิน
      data[i] = gray; // สีแดง
      data[i + 1] = gray; // สีเขียว
      data[i + 2] = gray; // สีน้ำเงิน
    }
    ...
    const channel = guild.channels.cache.get(process.env["CHANNEL_OLD_HUNTER"]);
    // const channel = guild.channels.cache.get(process.env["TEST_CHANNEL_EVENT"]);

    // ส่งรูป Canvas ในข้อความ
    if (channel) {
      channel.send({
        content: `**Hunter: ${member.displayName}**\nได้จากเราไปแล้วหวังว่าเราได้พบกันใหม่`,
        files: [attachment],
      });
    } else {
